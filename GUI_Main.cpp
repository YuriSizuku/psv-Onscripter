...
#include "vpkinstall/vitaPackage.h"
#include "vpkinstall/filesystem.h"
using namespace std;
...
#define Info_Rect_W 350
#define Info_Rect_H 544-40-Inter

#define Screen_X 15.0
#define Screen_Y 40.0
#define Screen_W 960.0-Screen_X*2
#define Screen_H 544.0-Screen_Y-Inter

#define Item_Rect_X Screen_X
#define Item_Rect_Y Screen_Y-Inter/4
#define Item_Rect_W Screen_W-Info_Rect_W-4
#define Item_Rect_H Icon_L+Inter/4
*/
...
void draw_title() {
	...
	sprintf(ver_str, "ONScripter-Jh for PSVita %s (GUI %d, Jh %s, %s, %d.%02d)\n", ONS_JH_VITA_VERSION, GUI_VERSION, ONS_JH_VERSION, ONS_VERSION, NSC_VERSION / 100, NSC_VERSION % 100);
...
void draw_config() {
	...
	//utils::printInfo("%s  %d\n", RomInfo::to_char(config.list_row), config.list_row);
	// FIXME: ugly UI
...
int  game_delete(int choose) {
	...
	/*int ret = removePath(rom_list[choose].path + "/");
	if (ret < 0) {
		utils::printInfo("Delete fail removePath() = 0x%08X\n", ret);
		return 0;
	}
	load_rom_list();*/
	return 1;
...
int  game_shortcut(int choose) {
	...
	if (checkFolderExist(path.c_str())) {
		ret = removePath(path);
		if (ret < 0) {
			utils::printInfo("removePath() = 0x%08X\n", ret);
		}
	}
...
int main() {
	...
	if (game_start_select >= 0 || unload_ui) {
		...
		cmd_str[cmd_num++] = (char*)(config.use_btouch == 0 ? "use_not_touch" : (config.use_btouch == 1 ? "use_front_only_touch" : "use_front_back_touch"));
		cmd_str[cmd_num] = NULL;
		sceAppMgrLoadExec("app0:ons.bin", cmd_str, NULL);
		//ons_main(cmd_num, cmd_str);//Game_Start
	}
}
...
/*
int draw_gui(vector<RomInfo_s> rom_list_s, bool &fontcache, bool &textbox) {

	char version_str[512] = { 0 };
	sprintf(version_str, "ONScripter-Jh for Vita version %s (Jh %s, %s, %d.%02d)\n", ONS_JH_VITA_VERSION, ONS_JH_VERSION, ONS_VERSION, NSC_VERSION / 100, NSC_VERSION % 100);

	SceCtrlData pad;

	vita2d_texture *image[256];
	vita2d_init();
	vita2d_set_clear_color(RGBA8(0x40, 0x40, 0x40, 0xFF));
	font = vita2d_load_font_file("ux0:onsemu/default.ttf");
	char icon_name[256];
	int num = rom_list_s.size();
	for (int i = 0; i < num; i++) {
		image[i] = vita2d_load_PNG_file((rom_list_s[i].path + "/icon.png").c_str());
		if (image[i] == NULL)
			image[i] = vita2d_create_empty_texture(150, 150);
	}
	memset(&pad, 0, sizeof(pad));
	char temp_char[10];
	int index = 0;
	int Head_index = 0;
	int Display_num = (Screen_H) / (Item_Rect_H + Inter);
	while (1) {
		sceCtrlPeekBufferPositive(0, &pad, 1);
		if (pad.buttons & SCE_CTRL_START)//STRAR\CD˳\F6
			end();
		if (pad.buttons & SCE_CTRL_L2)//fontcache
		{
			fontcache = !fontcache;
		}
		if (pad.buttons & SCE_CTRL_R2)//textbox
		{
			textbox = !textbox;
		}
		if (pad.buttons & SCE_CTRL_UP && num > 0) {
			index--;
			//index 8    Head_index 8  Display_num 4   num 12
			if (Head_index > 0 && Head_index > index) {
				Head_index--;
			}
			if (index < 0) {
				index = num - 1;
				Head_index = num - Display_num;
			}
		}
		if (pad.buttons & SCE_CTRL_DOWN && num > 0) {
			index++;
			if (Head_index <num - Display_num && index - Head_index > Display_num - 1) {
				Head_index++;
			}
			if (index >= num) {
				index = 0;
		...
		if (pad.buttons & SCE_CTRL_CIRCLE && num > 0) {//ȷ\B6\A8
			utils::printInfo("   run==>%s\n", rom_list_s[index].path.c_str());
			break;
		}

		vita2d_start_drawing();
		vita2d_clear_screen();
		int rect_y = Screen_Y + (Item_Rect_H + Inter)*Display_num;
		vita2d_draw_line(Screen_X, Screen_Y - Inter, Screen_X + Screen_W, Screen_Y - Inter, RGBA8(255, 255, 255, 255));//top
		vita2d_draw_line(Screen_X, rect_y, Screen_X + Inter / 2 + Screen_W, rect_y, RGBA8(255, 255, 255, 255));//down

		vita2d_draw_line(Screen_X, Screen_Y - Inter, Screen_X, rect_y, RGBA8(255, 255, 255, 255));//left
		vita2d_draw_line(Screen_X + Screen_W, Screen_Y - Inter, Screen_X + Screen_W, Screen_Y + (Item_Rect_H + Inter)*Display_num, RGBA8(255, 255, 255, 255));//right

		vita2d_draw_line(Screen_X + Screen_W - Info_Rect_W, Screen_Y - Inter, Screen_X + Screen_W - Info_Rect_W, rect_y, RGBA8(255, 255, 255, 255));//mut


		if (num > 0) {
			//select------------
			vita2d_draw_rectangle(Item_Rect_X + 3, Item_Rect_Y + (index - Head_index) * (Item_Rect_H + Inter), Item_Rect_W, Item_Rect_H, RGBA8(0, 0, 255, 255));
			vita2d_draw_texture_scale(image[index], Screen_X + Screen_W - Info_Rect_W + 25, Screen_Y + 25,
				(Info_Rect_W - 50) / (float)sceGxmTextureGetWidth(&image[index]->gxm_tex),
				(Info_Rect_W - 50) / (float)sceGxmTextureGetHeight(&image[index]->gxm_tex));

			//all----------------
			for (int i = Head_index; i < Head_index + Display_num; i++) {
				if (i >= num) break;
				int temp_y = Screen_Y + (i - Head_index) * (Item_Rect_H + Inter);
				snprintf(temp_char, 10, "%02d", i + 1);
				vita2d_font_draw_text(font, Screen_X + 5, temp_y + 25, RGBA8(0, 255, 0, 255), FONT_SIZE, temp_char);
				vita2d_draw_texture_scale(image[i], Screen_X + 40, temp_y,
					Icon_L / (float)sceGxmTextureGetWidth(&image[i]->gxm_tex),
					Icon_L / (float)sceGxmTextureGetHeight(&image[i]->gxm_tex));
				vita2d_font_draw_text(font, Screen_X + 40 + Icon_L + 10, temp_y + 25, RGBA8(0, 255, 0, 255), FONT_SIZE, rom_list_s[i].path.c_str());
				vita2d_font_draw_text(font, Screen_X + 40 + Icon_L + 10, temp_y + 55, RGBA8(0, 255, 0, 255), FONT_SIZE, rom_list_s[i].name.c_str());

			}
		}

		vita2d_font_draw_text(font, Screen_X + 5, 5 + 20, RGBA8(0, 255, 0, 255), FONT_SIZE, version_str);

		vita2d_font_draw_text(font, Screen_X + 5, rect_y + 20, RGBA8(0, 255, 0, 255), FONT_SIZE, "[L]Font Cache(Some image is not displayed, select Off):");
		vita2d_font_draw_text(font, Screen_X + 570, rect_y + 20, RGBA8(0, 255, 0, 255), FONT_SIZE, fontcache ? "ON" : "OFF");

		vita2d_font_draw_text(font, Screen_X + 5, rect_y + 50, RGBA8(0, 255, 0, 255), FONT_SIZE, "[R]TextBox Background(Black block appears, select Off):");
		vita2d_font_draw_text(font, Screen_X + 570, rect_y + 50, RGBA8(0, 255, 0, 255), FONT_SIZE, textbox ? "ON" : "OFF");


		vita2d_end_drawing();
		vita2d_swap_buffers();
		sceKernelDelayThread(1000 * 125);

	}
	vita2d_fini();
	for (int i = 0; i < num; i++)
		vita2d_free_texture(image[i]);
	vita2d_free_font(font);
	//vita2d_free_pgf(pgf);
...
#if defined(TEST)
	ret = debugNetInit(ip_server, port_server, INFO);
#endif

	

	vector<RomInfo_s> rom_list_s;
	utils::printInfo("ONScripter-Jh for Vita version %s (Jh %s, %s, %d.%02d)\n", ONS_JH_VITA_VERSION, ONS_JH_VERSION, ONS_VERSION, NSC_VERSION / 100, NSC_VERSION % 100);
START:
			...
			do {
				...
				if (res > 0) {
					...
					if (SCE_S_ISDIR(dir.d_stat.st_mode)) {
						rom_list_s.push_back({ temp,temp2 });
					}
				}
			} while (res > 0);
		}
	...
	sceIoDclose(dfd);
	bool fontcache = true, textbox = true;
	int rom_index = draw_gui(rom_list_s, fontcache, textbox);
	utils::printInfo("ONScripter-jh for PSVita\n");
	if (rom_index != -1) {
		string str = rom_list_s[rom_index].path + "/ons.ini";
		int len = str.length();
		config_path = new char[len + 1];
		strcpy(config_path, str.c_str());
	}
	char* command[10];
	int _num = 0;

	command[_num] = (char*)"ons";
	_num++;
	command[_num] = (char*)"--root";
	_num++;

	char* dirpath;
	int len = rom_list_s[rom_index].path.length();
	dirpath = new char[len + 1];
	strcpy(dirpath, rom_list_s[rom_index].path.c_str());
	command[_num] = dirpath;
	_num++;
	command[_num] = (char*)"--compatible";
	_num++;
	if (fontcache) {
		command[_num] = (char*)"--fontcache";
		_num++;
	}
	if (textbox) {
		command[_num] = (char*)"--textbox";
		_num++;
	}

	//command[5] = (char*)"--debug:1";


	ons_main(_num, command);
	goto START;
